...
export function AdminShell({ children }: { children: ReactNode }) {
  const [location] = useLocation();
  const [, navigate] = useLocation();
  const items = [['/admin/applications/pending', 'Hồ sơ đăng ký', ClipboardCheck], ['/admin/criteria', 'Cấu hình tiêu chí', Settings2], ['/admin/accounts', 'Quản lý tài khoản', ShieldCheck], ['/admin/reports', 'Báo cáo thống kê', BarChart3]];
  const logout = () => {
    sessionStorage.removeItem('attp-reviewer-session');
    sessionStorage.removeItem('attp-session-role');
  ...
  return <div className="portal-noise min-h-[100dvh] bg-[#f5f7f8] lg:grid lg:grid-cols-[252px_1fr]">
      <aside className="hidden min-h-[100dvh] bg-[#123d36] text-sidebar-foreground lg:flex lg:flex-col"><div className="border-b border-white/10 px-5 py-5"><Link href="/" className="focus-ring flex items-center gap-3" data-testid="link-admin-brand-home"><img src={emblemPath} alt="Quốc huy Việt Nam" className="h-12 w-12 object-contain" /><span className="leading-tight"><strong className="block text-[11px] font-extrabold tracking-tight text-white">SỞ AN TOÀN THỰC PHẨM</strong><span className="mt-1 block text-[10px] font-semibold text-white/60">THÀNH PHỐ HỒ CHÍ MINH</span><span className="mt-3 block text-[10px] font-bold uppercase tracking-[.16em] text-[#f4c95d]">Không gian quản trị</span></span></Link></div><nav className="flex-1 space-y-1.5 p-4" aria-label="Điều hướng quản trị">{items.map(([href, label, Icon]) => <Link key={href as string} href={href as string} className={`focus-ring group flex items-center gap-3 rounded-2xl px-3.5 py-3 text-sm font-semibold transition-all ${location.startsWith(href as string) ? 'bg-[#f4c95d] text-[#123d36] shadow-lg shadow-black/10' : 'text-white/65 hover:bg-white/[.08] hover:text-white'}`} data-testid={`link-admin-${label}`}><span className={`flex h-8 w-8 items-center justify-center rounded-xl transition-colors ${location.startsWith(href as string) ? 'bg-[#123d36]/10' : 'bg-white/[.06] group-hover:bg-white/10'}`}><Icon size={17} /></span><span>{label as string}</span></Link>)}</nav><div className="border-t border-white/10 p-5 text-xs text-white/45">Phiên làm việc cán bộ<br /><span className="mt-1 inline-block font-semibold text-white/75">Phòng Quản lý cơ sở</span></div></aside>
