...
export function HomePage() {
  ...
  return <PublicShell><main>
    <section className="portal-grid relative overflow-hidden border-b border-border"><div className="mx-auto grid max-w-7xl gap-14 px-5 py-20 lg:grid-cols-[1.02fr_.98fr] lg:items-center lg:px-8 lg:py-28">
      ...
      <div className="relative flex items-center justify-center lg:justify-end"><div className="absolute -right-8 top-0 h-72 w-72 rounded-full bg-accent/25 blur-3xl" /><div className="relative w-full max-w-lg rotate-1 rounded-[2rem] border border-primary/15 bg-card p-3 shadow-2xl shadow-primary/15 transition-transform duration-500 hover:rotate-0"><div className="relative overflow-hidden rounded-[1.5rem]"><img src={heroFoodImage} alt="Mâm thực phẩm tươi ngon với rau củ, trái cây và các loại hạt" className="aspect-[1.05/1] w-full object-cover" data-testid="img-home-hero-food" /><div className="absolute inset-x-4 bottom-4 flex items-center justify-between rounded-2xl border border-white/25 bg-primary/85 px-4 py-3 text-primary-foreground shadow-lg backdrop-blur-md"><div><p className="mono-label text-accent">LỰA CHỌN AN TOÀN</p><p className="mt-1 text-sm font-bold">Từ nguồn gốc đến bàn ăn</p></div><ShieldCheck size={25} className="text-accent" /></div></div></div></div>
    </div></section>
...
function ApplicationForm() {
  ...
  const submit = (event: FormEvent) => {
    event.preventDefault();
     ...
     const input: ApplicationInput = { type, applicantName: fields.applicantName ?? '', address: fields.address ?? '', contact: fields.contact ?? '', criteriaVersion: criteriaSet?.version ?? '', isThirdParty: type === 'school' && fields.mealModel !== 'Tự nấu' && fields.supplierSource === 'manual', data, attachments: files };
...
export function AdminCriteriaPage() {
  ...
  useEffect(() => {
    ...
    setDraft(criteriaSet.criteria);
    setEffectiveFrom(criteriaSet.effectiveFrom);
  }, [criteriaSet?.version]);

  const ordinaryTotal = draft.filter(item => item.active && !item.prerequisite).reduce((sum, item) => sum + Number(item.maxScore || 0), 0);
  ...
  const addItem = () => {
    ...
    setDraft(items => [...items, { id: `custom-${Date.now()}`, key: `custom_${items.length + 1}`, label: 'Tiêu chí mới', description: '', groupId: groups[0].id, answerType: 'text', options: [], maxScore: 0, required: true, prerequisite: false, active: true, order: items.length + 1, sourceMaterials: [] }]);
  };
  ...
  const save = () => {
    ...
    updateCriteriaMutation.mutate({ type, data: { groups, criteria: draft, effectiveFrom, applyMode } }, {
  ...
  return <AdminShell><div className="mx-auto max-w-7xl px-5 py-9 lg:px-10">
    <SectionHeading eyebrow="Cấu hình gốc của biểu mẫu" title="Bộ tiêu chí & form tự sinh" description="Tiêu chí là nguồn dữ liệu gốc. Mỗi thay đổi tạo một phiên bản mới để hồ sơ cũ vẫn giữ đúng bộ tiêu chí đã dùng khi nộp." action={<Link href="/register" className="inline-flex items-center gap-2 text-sm font-bold text-primary">Xem form đang sinh <ArrowUpRight size={16} /></Link>} />
    <div className="flex flex-col gap-3 rounded-2xl border border-border bg-card p-4 shadow-sm sm:flex-row sm:items-center sm:justify-between"><label className="flex items-center gap-3 text-sm font-bold">Loại hình<select value={type} onChange={event => setType(event.target.value as ApplicationType)} className="focus-ring h-10 rounded-xl border border-input bg-background px-3 font-semibold" data-testid="select-criteria-type"><option value="food-supplier">Cung cấp thực phẩm</option><option value="meal-provider">Cung cấp suất ăn</option><option value="school">Cơ sở giáo dục</option></select></label><div className={`rounded-xl px-4 py-2 text-sm font-bold ${ordinaryTotal === 100 ? 'bg-emerald-100 text-emerald-900' : 'bg-red-100 text-red-900'}`} data-testid="criteria-total-score">Tổng điểm: {ordinaryTotal}/100 {ordinaryTotal === 100 ? '· Hợp lệ' : '· Chưa thể lưu'}</div></div>
     <div className="mt-6 rounded-2xl border border-primary/15 bg-secondary/30 p-5"><div className="flex flex-col gap-2 sm:flex-row sm:items-center sm:justify-between"><div><p className="mono-label text-primary">Bước 1 · Tạo nhóm</p><p className="mt-1 text-sm text-muted-foreground">Mỗi nhóm có mã riêng. Tiêu chí chỉ được chọn từ danh sách nhóm này, không nhập tên nhóm tự do.</p></div><button type="button" onClick={addGroup} className="inline-flex h-10 items-center justify-center gap-2 rounded-xl bg-primary px-4 text-sm font-bold text-primary-foreground"><Plus size={16} /> Tạo nhóm</button></div><div className="mt-4 grid gap-3 md:grid-cols-2">{groups.map((group, index) => <div key={group.id} className="flex items-center gap-3 rounded-xl border border-border bg-card p-3"><span className="font-mono text-xs font-bold text-primary">{String(index + 1).padStart(2, '0')}</span><input value={group.name} onChange={event => updateGroup(group.id, { name: event.target.value })} className="focus-ring h-10 min-w-0 flex-1 rounded-lg border border-input bg-background px-3 text-sm font-semibold" aria-label={`Tên nhóm ${index + 1}`} data-testid={`input-criteria-group-${group.id}`} /><span className="shrink-0 rounded-md bg-muted px-2 py-1 font-mono text-[10px] text-muted-foreground">{group.id}</span></div>)}</div></div>
    <div className="mt-6 grid gap-6 lg:grid-cols-[1fr_320px]">
      <div className="space-y-4">
         {draft.map((item, index) => <div key={item.id} className={`rounded-2xl border bg-card p-5 shadow-sm ${item.active ? 'border-border' : 'border-dashed border-border/70 opacity-60'}`} data-testid={`criteria-row-${item.key}`}>
           <div className="flex flex-col gap-4 lg:flex-row lg:items-start"><div className="flex-1"><div className="flex flex-wrap items-center gap-2"><span className="rounded-full bg-secondary px-2 py-1 font-mono text-xs font-bold text-primary">{String(index + 1).padStart(2, '0')}</span>{item.prerequisite && <span className="rounded-full bg-red-100 px-2 py-1 text-xs font-bold text-red-900">Điều kiện tiên quyết</span>}{!item.active && <span className="rounded-full bg-muted px-2 py-1 text-xs font-bold text-muted-foreground">Đang ẩn</span>}</div><div className="mt-4 grid gap-3 md:grid-cols-2"><label className="md:col-span-2"><span className="mb-1 block text-xs font-bold text-muted-foreground">Tên tiêu chí / câu hỏi</span><input value={item.label} onChange={event => updateItem(item.id, { label: event.target.value })} className="focus-ring h-10 w-full rounded-lg border border-input bg-background px-3 text-sm font-semibold" /></label><label><span className="mb-1 block text-xs font-bold text-muted-foreground">Nhóm đã tạo</span><select value={item.groupId} onChange={event => updateItem(item.id, { groupId: event.target.value })} className="focus-ring h-10 w-full rounded-lg border border-input bg-background px-3 text-sm"><option value="">Chọn nhóm</option>{groups.map(group => <option key={group.id} value={group.id}>{group.name}</option>)}</select></label><label><span className="mb-1 block text-xs font-bold text-muted-foreground">Loại trả lời</span><select value={item.answerType} onChange={event => updateItem(item.id, { answerType: event.target.value as CriteriaAnswerType })} className="focus-ring h-10 w-full rounded-lg border border-input bg-background px-3 text-sm"><option value="text">Nhập text</option><option value="number">Nhập số</option><option value="date">Ngày</option><option value="yes-no">Có / Không</option><option value="select">Danh sách</option><option value="file">Upload file / ảnh</option></select></label><label><span className="mb-1 block text-xs font-bold text-muted-foreground">Điểm tối đa</span><input type="number" min="0" value={item.maxScore} disabled={item.prerequisite} onChange={event => updateItem(item.id, { maxScore: Math.max(0, Number(event.target.value)) })} className="focus-ring h-10 w-full rounded-lg border border-input bg-background px-3 text-sm font-semibold disabled:bg-muted" /></label><label className="flex items-center gap-2 self-end pb-2 text-sm font-semibold"><input type="checkbox" checked={item.required} onChange={event => updateItem(item.id, { required: event.target.checked })} /> Bắt buộc trả lời</label></div><label className="mt-4 block"><span className="mb-1 block text-xs font-bold text-muted-foreground">Mô tả / hướng dẫn</span><textarea value={item.description} onChange={event => updateItem(item.id, { description: event.target.value })} className="focus-ring min-h-20 w-full rounded-lg border border-input bg-background px-3 py-2 text-sm" placeholder="Nêu rõ cần khai báo hoặc cung cấp minh chứng gì..." /></label><div className="mt-4 rounded-xl border border-dashed border-primary/25 bg-secondary/30 p-3"><div className="flex flex-wrap items-center justify-between gap-2"><div><p className="text-sm font-bold">Tài liệu căn cứ</p><p className="text-xs text-muted-foreground">Upload PDF/JPG/PNG để form biết tiêu chí dựa trên tài liệu nào.</p></div><label className="focus-ring inline-flex cursor-pointer items-center gap-2 rounded-lg border border-primary/25 px-3 py-2 text-xs font-bold text-primary hover:bg-secondary"><Plus size={15} /> Thêm tài liệu<input type="file" accept=".pdf,.jpg,.jpeg,.png,application/pdf,image/jpeg,image/png" multiple onChange={event => addSourceMaterials(item.id, event)} className="sr-only" data-testid={`input-criteria-source-${item.key}`} /></label></div>{item.sourceMaterials.length > 0 && <div className="mt-3 space-y-2">{item.sourceMaterials.map(source => <div key={source.id} className="flex items-center justify-between gap-3 rounded-lg bg-card px-3 py-2 text-xs"><span className="flex min-w-0 items-center gap-2 truncate"><FileText size={14} className="shrink-0 text-primary" />{source.name}</span><button type="button" onClick={() => removeSourceMaterial(item.id, source.id)} className="shrink-0 font-bold text-muted-foreground hover:text-destructive">Xóa</button></div>)}</div>}</div></div><div className="flex shrink-0 gap-2 lg:flex-col"><button type="button" onClick={() => updateItem(item.id, { prerequisite: !item.prerequisite, maxScore: item.prerequisite ? item.maxScore || 10 : 0 })} className="rounded-lg border border-border px-3 py-2 text-xs font-bold hover:bg-secondary">{item.prerequisite ? 'Bỏ điều kiện' : 'Đặt tiên quyết'}</button><button type="button" onClick={() => hideItem(item.id)} disabled={!item.active} className="rounded-lg border border-border px-3 py-2 text-xs font-bold text-destructive hover:bg-red-50 disabled:opacity-50">Ẩn tiêu chí</button></div></div>
        </div>)}
        <button type="button" onClick={addItem} className="flex w-full items-center justify-center gap-2 rounded-2xl border border-dashed border-primary/35 bg-secondary/30 py-4 text-sm font-bold text-primary hover:bg-secondary"><Plus size={17} /> Thêm tiêu chí</button>
      </div>
      <aside className="h-fit space-y-4 lg:sticky lg:top-24"><div className="rounded-2xl border border-border bg-card p-5 shadow-sm"><p className="mono-label text-primary">Phiên bản hiện tại</p><p className="mt-2 text-2xl font-extrabold">{criteriaSet.version}</p><p className="mt-1 text-sm text-muted-foreground">Áp dụng từ {formatDate(criteriaSet.effectiveFrom)}</p><label className="mt-5 block text-sm font-semibold">Ngày áp dụng mới<input type="date" value={effectiveFrom} onChange={event => setEffectiveFrom(event.target.value)} className="focus-ring mt-2 h-10 w-full rounded-lg border border-input bg-background px-3 text-sm" /></label><label className="mt-4 block text-sm font-semibold">Cách áp dụng<select value={applyMode} onChange={event => setApplyMode(event.target.value as CriteriaConfigInput['applyMode'])} className="focus-ring mt-2 h-10 w-full rounded-lg border border-input bg-background px-3 text-sm"><option value="now">Áp dụng ngay</option><option value="scheduled">Áp dụng từ ngày đã chọn</option></select></label><button type="button" onClick={save} disabled={ordinaryTotal !== 100 || updateCriteriaMutation.isPending} className="mt-5 inline-flex h-11 w-full items-center justify-center gap-2 rounded-xl bg-primary px-4 text-sm font-bold text-primary-foreground disabled:cursor-not-allowed disabled:opacity-50"><Check size={16} />{updateCriteriaMutation.isPending ? 'Đang lưu…' : 'Lưu phiên bản tiêu chí'}</button></div><div className="rounded-2xl border border-border bg-card p-5 shadow-sm"><p className="mono-label text-primary">Lịch sử thay đổi</p><div className="mt-4 space-y-3">{history?.length ? history.slice(0, 5).map(entry => <div key={entry.id} className="border-l-2 border-primary/30 pl-3"><p className="text-xs font-bold">{entry.summary}</p><p className="mt-1 text-xs text-muted-foreground">{entry.changedBy} · {formatDate(entry.changedAt)} · {entry.version}</p></div>) : <p className="mt-3 text-sm text-muted-foreground">Chưa có thay đổi trong phiên làm việc này.</p>}</div></div></aside>
    </div>
...
function CriteriaReviewPanel({ criteria, criteriaGroups, data, scores, prerequisites, notes, setScore, setPrerequisite, setNote }: { criteria: CriteriaDefinition[]; criteriaGroups: CriteriaGroup[]; data: Record<string, unknown>; scores: Record<string, number>; prerequisites: Record<string, boolean>; notes: Record<string, string>; setScore: (key: string, value: number) => void; setPrerequisite: (key: string, value: boolean) => void; setNote: (key: string, value: string) => void }) {
  const groups = criteriaGroups.slice().sort((a, b) => a.order - b.order).filter(group => criteria.some(item => item.groupId === group.id));
  return <div className="rounded-2xl border border-border bg-card p-6"><div className="flex items-center justify-between gap-3"><div className="flex items-center gap-3"><ClipboardCheck size={19} className="text-primary" /><h2 className="text-lg font-extrabold">Chấm từng tiêu chí</h2></div><span className="rounded-full bg-secondary px-3 py-1 text-xs font-bold text-primary">{criteria.length} tiêu chí trong snapshot</span></div><p className="mt-2 text-sm leading-6 text-muted-foreground">Điều kiện tiên quyết chỉ ghi nhận đạt / không đạt. Tiêu chí thường cho phép điểm một phần và được cộng tự động.</p><div className="mt-5 space-y-6">{groups.map(group => <div key={group.id}><p className="mono-label border-b border-border pb-2 text-primary">Nhóm {group.name}</p><div className="mt-3 space-y-3">{criteria.filter(item => item.groupId === group.id).map(item => <div key={item.key} className="rounded-xl border border-border/80 bg-secondary/20 p-4"><div className="flex flex-col gap-3 md:flex-row md:items-start md:justify-between"><div className="min-w-0"><p className="font-bold">{item.label}{item.required && <span className="text-destructive"> *</span>}</p><p className="mt-1 break-words text-sm text-muted-foreground">Khai báo: {formatAnswer(data[item.key])}</p>{item.sourceMaterials.length > 0 && <p className="mt-2 text-xs text-muted-foreground"><span className="font-bold text-primary">Căn cứ:</span> {item.sourceMaterials.map(source => source.name).join(', ')}</p>}</div>{item.prerequisite ? <div className="flex shrink-0 gap-2"><button type="button" onClick={() => setPrerequisite(item.key, true)} className={`rounded-lg px-3 py-2 text-xs font-bold ${prerequisites[item.key] === true ? 'bg-emerald-600 text-white' : 'border border-border bg-background'}`}>Đạt</button><button type="button" onClick={() => setPrerequisite(item.key, false)} className={`rounded-lg px-3 py-2 text-xs font-bold ${prerequisites[item.key] === false ? 'bg-red-600 text-white' : 'border border-border bg-background'}`}>Không đạt</button></div> : <div className="flex shrink-0 items-center gap-2"><input aria-label={`Điểm ${item.label}`} type="number" min="0" max={item.maxScore} value={scores[item.key] ?? 0} onChange={event => setScore(item.key, Math.min(item.maxScore, Math.max(0, Number(event.target.value))))} className="focus-ring h-10 w-20 rounded-lg border border-input bg-background px-2 text-right font-mono font-bold" /><span className="font-mono text-sm text-muted-foreground">/ {item.maxScore}</span></div>}</div><label className="mt-3 block"><span className="mb-1 block text-xs font-semibold text-muted-foreground">Ghi chú tiêu chí</span><input value={notes[item.key] ?? ''} onChange={event => setNote(item.key, event.target.value)} placeholder="Nhập nhận xét / căn cứ kiểm tra" className="focus-ring h-9 w-full rounded-lg border border-input bg-background px-3 text-sm" /></label></div>)}</div></div>)}</div></div>;
}
...
export function AdminApplicationPage() {
  ...
  useEffect(() => {
    ...
    setPrerequisites(application.prerequisiteResults ?? Object.fromEntries((application.criteriaSnapshot ?? []).filter(item => item.prerequisite).map(item => [item.key, true])));
  ...
  const ordinaryCriteria = criteria.filter(item => !item.prerequisite);
